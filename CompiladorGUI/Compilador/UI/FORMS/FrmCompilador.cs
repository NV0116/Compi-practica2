...
namespace Compilador.UI.Forms
{
    public partial class FrmCompilador : Form
    {
        ...
        private void btnCompilar_Click(object sender, EventArgs e)
        {
            ...
            // Imprimir errores en el estatus
            foreach (var aviso in resultado.Avisos)
            {
                txtEstatus.AppendText(aviso + Environment.NewLine);
            }

            // Imprimir tokens con el formato: [Linea] ID Lexema
            foreach (var t in resultado.Tokens)
            {
                txtTokens.AppendText($"[{t.Linea}] {t.Tipo} {t.Lexema}{Environment.NewLine}");
            }

            txtEstatus.AppendText("Analisis Lexico finalizado con éxito");
